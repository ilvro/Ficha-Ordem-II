...
function triggerGlitchTransition(callback){if(_glitchActive){callback();return}_glitchActive=true;playGlitchSound();const contents=document.querySelector('.cinematic-contents');const doEnter=()=>{callback();requestAnimationFrame(()=>{const next=document.querySelector('.cinematic-contents');if(next){next.classList.add('glitch-enter');const done=()=>{next.classList.remove('glitch-enter');_glitchActive=false};next.addEventListener('animationend',done,{once:true});setTimeout(()=>{next.classList.remove('glitch-enter');_glitchActive=false},500)}else{_glitchActive=false}})};if(contents){contents.classList.add('glitch-exit');setTimeout(doEnter,115)}else{doEnter()}}
