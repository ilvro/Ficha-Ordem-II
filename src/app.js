...
function applySlotValue(value){if(!activeSlotTarget||value===null||value===undefined||value==='')return;const{card,marker,slotIndex}=activeSlotTarget,costRes=(marker.fillCostResource||'pd').toLowerCase(),costAmt=Number(marker.fillCostAmount)||0;if(costAmt>0){const currentVal=state[costRes]??0;if(currentVal<costAmt){const toast=document.querySelector('#save-toast');if(toast){toast.textContent=`${costRes.toUpperCase()} INSUFICIENTE (NECESSÁRIO ${costAmt})`;toast.classList.add('show');setTimeout(()=>{toast.classList.remove('show');toast.textContent='SALVO'},1800)}return}state[costRes]-=costAmt;playResourceSound(false)}if(!Array.isArray(marker.slots))marker.slots=[];marker.slots[slotIndex]=Number(value)||value;marker.current=marker.slots.filter(v=>v!==null&&v!==undefined&&v!=='').length;closeSlotModal();save(false);render();const toast=document.querySelector('#save-toast');if(toast){toast.textContent=costAmt>0?`-${costAmt} ${costRes.toUpperCase()} · ESPAÇO ${slotIndex+1} PREENCHIDO COM [ ${value} ]!`:`ESPAÇO ${slotIndex+1} PREENCHIDO COM [ ${value} ]!`;toast.classList.add('show');setTimeout(()=>{toast.classList.remove('show');toast.textContent='SALVO'},1800)}}
