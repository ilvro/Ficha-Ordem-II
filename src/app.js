...
function cinematicSection(key,title,content,addSchema=''){const isLockable=key==='items'||key==='history';const collapsed=(presentationMode&&isLockable)?true:!!state.cinematicCollapsed?.[key];const lockHtml=isLockable?`<i class="cinematic-section-lock" title="Bloqueado no modo apresentação" aria-hidden="true">${icon('lock',15)}</i>`:'';return `<section class="cinematic-section-group ${collapsed?'collapsed':''} ${isLockable?'section-lockable':''}"><div class="cinematic-section-heading"><button class="cinematic-section-toggle" data-cinematic-section-toggle="${key}" aria-expanded="${!collapsed}"><span>${lockHtml}${title}</span>${icon('chevron',18)}</button>${editMode&&addSchema?`<button class="cinematic-section-add" data-cinematic-add="${addSchema}" title="Adicionar">${icon('plus',15)}</button>`:''}</div><div class="cinematic-section-body"><div>${content}</div></div></section>`}
...
function confirmPresentationAgent(chosenSheet){
 ...
 setTimeout(()=>{
  ...
  activeSheetId=state.id;
  state.viewMode='cinematic';
  if(state.cinematicCollapsed){
   state.cinematicCollapsed.items=true;
   state.cinematicCollapsed.history=true;
  }
  save(false);
  render();
 },540);
