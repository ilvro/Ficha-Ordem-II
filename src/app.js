...
function triggerGlitchTransition(callback){if(_glitchActive){callback();return}_glitchActive=true;playGlitchSound();const overlay=document.createElement('div');overlay.className='glitch-blackout-overlay';document.body.appendChild(overlay);requestAnimationFrame(()=>{callback();requestAnimationFrame(()=>{const contents=document.querySelector('.cinematic-contents');if(contents){contents.classList.add('glitch-enter');const done=()=>{contents.classList.remove('glitch-enter');contents.removeEventListener('animationend',done)};contents.addEventListener('animationend',done,{once:true})}})});setTimeout(()=>{overlay.remove();_glitchActive=false},340)}
