...
function normalizeSheet(raw={}){
 ...
 sheet.cinematicOutlineWidth=Math.max(0,Math.min(10,Number(raw.cinematicOutlineWidth??2)));sheet.cinematicOutlineFade=Math.max(0,Math.min(10,Number(raw.cinematicOutlineFade??1.2)));
...
function syncTokenOutlineFilter(){const flood=document.querySelector('#token-outline-flood'),morph=document.querySelector('#token-outline-morph'),blur=document.querySelector('#token-outline-blur'),color=state.diceColor||PROFILE_COLORS[state.profile]||PROFILE_COLORS.Executor;if(flood)flood.setAttribute('flood-color',color);const width=Number(state.cinematicOutlineWidth??2),fade=Number(state.cinematicOutlineFade??1.2);if(morph)morph.setAttribute('radius',String(width));if(blur)blur.setAttribute('stdDeviation',String(fade));const tokenImg=document.querySelector('.cinematic-character>img');if(tokenImg)tokenImg.classList.toggle('no-outline',width<=0)}
...
function renderCinematic(trainingOptions,bgIsVideo=false){const accent=PROFILE_COLORS[state.profile]||PROFILE_COLORS.Executor,profileOptions=['Executor','Analista','Vigilante'].map(value=>({value,label:value})),abilities=visibleCards('abilities').map(renderCinematicCard).join('')||'<p class="cinematic-empty">Nenhuma habilidade.</p>',items=visibleCards('items').map(renderCinematicCard).join('')||'<p class="cinematic-empty">Nenhum item.</p>',history=editMode?`<textarea class="cinematic-history" data-field="history" placeholder="História do personagem...">${safe(state.history||'')}</textarea>`:`<p class="cinematic-history-copy">${formatCardText(state.history||'Nenhuma história registrada.')}</p>`,identity=editMode?`<div class="cinematic-identity cinematic-identity-edit">${customSelect({value:state.profile,options:profileOptions,id:'profile',className:'cinematic-profile-select'})}<i></i><input class="cinematic-occupation-input" data-field="occupation" value="${safe(state.occupation)}"><i></i><span class="cinematic-level-label">NÍVEL</span><input class="cinematic-level-input" data-field="level" value="${state.level}" inputmode="numeric"></div>`:`<div class="cinematic-identity-read"><strong>${safe(state.profile)}</strong><i></i><span>${safe(state.occupation)}</span><i></i><em>NÍVEL</em><b>${state.level}</b></div>`,aptidaoOptions=APTIDAO_FIELDS.map(v=>({value:v,label:v})),aptidaoCount=state.skills.filter(s=>s.name.startsWith('APTIDÃO')).length;return `<main class="cinematic-page ${modeJustChanged?'mode-enter':''}${bgIsVideo?' has-video-bg':''}" style="--cinematic-accent:${accent};--resource-shift:${currentResourceShift}px">${bgIsVideo?'<video class="bg-video" autoplay loop muted playsinline></video><div class="bg-overlay"></div>':''}<div class="cinematic-frame"><i class="cinematic-frame-line top"><b></b><b></b></i><i class="cinematic-frame-line bottom"><b></b><b></b></i></div><header class="cinematic-header"><div class="cinematic-name">${editMode?`<input data-field="name" value="${safe(state.name)}">`:`<h1>${safe(state.name)}</h1>`}</div>${identity}</header>
 ...
 <section class="cinematic-character" data-cinematic-character>${state.tokenImage?`<img src="${safeUrl(state.tokenImage)}" alt="${safe(state.name)}" draggable="false" data-cinematic-token style="left:${state.cinematicTokenX}%;top:${state.cinematicTokenY}%;--token-scale:${state.cinematicTokenScale/100}" class="${(state.cinematicOutlineWidth??2)<=0?'no-outline':''}">`:`<div class="cinematic-token-empty"><span>${icon('user',54)}</span><p>ADICIONE A IMAGEM DO PERSONAGEM</p></div>`}${editMode?`<div class="cinematic-token-tools"><label>${icon('image',15)} PERSONAGEM<input id="token-upload-cinematic" type="file" accept="image/*"></label>${state.tokenImage?`<button type="button" class="btn-upscale-token" data-upscale-token title="Aprimorar resolução e nitidez nativamente">${icon('sparkles',14)} UPSCALE 2X (HD)</button>`:''}<label>ESCALA <input type="range" min="45" max="220" value="${state.cinematicTokenScale}" data-cinematic-scale></label><label title="Espessura do contorno">CONTORNO <input type="range" min="0" max="6" step="0.5" value="${state.cinematicOutlineWidth??2}" data-cinematic-outline-width></label><label title="Fade / suavidade da borda">FADE <input type="range" min="0.2" max="4" step="0.2" value="${state.cinematicOutlineFade??1.2}" data-cinematic-outline-fade></label><button data-reset-cinematic-token>RECENTRALIZAR</button><label>${icon('image',15)} FUNDO<input id="background-upload" type="file" accept="image/*,video/mp4,video/webm"></label>${state.backgroundImage?'<button data-reset-background>FUNDO PADRÃO</button>':''}<label>COR DOS DADOS <input id="dice-color" type="color" value="${safe(state.diceColor||PROFILE_COLORS[state.profile])}"></label>${state.diceColor?'<button data-reset-dice-color>COR DO PERFIL</button>':''}<small>Arraste o personagem para reposicionar · Contorno com fade suave</small></div>`:''}</section>
...
async function upscaleAndEnhanceImage(source,options={}){if(!source)return null;const{scale=2,sharpness=0.65,denoiseAlpha=true}=options;return new Promise((resolve,reject)=>{const img=new Image();img.crossOrigin='anonymous';img.onload=()=>{try{const origW=img.naturalWidth||img.width,origH=img.naturalHeight||img.height,actualScale=Math.min(scale,2400/Math.max(origW,origH)),targetW=Math.max(1,Math.round(origW*actualScale)),targetH=Math.max(1,Math.round(origH*actualScale)),canvas=document.createElement('canvas');canvas.width=targetW;canvas.height=targetH;const ctx=canvas.getContext('2d',{willReadFrequently:true});ctx.imageSmoothingEnabled=true;ctx.imageSmoothingQuality='high';ctx.drawImage(img,0,0,targetW,targetH);const imgData=ctx.getImageData(0,0,targetW,targetH),data=imgData.data,copy=new Uint8ClampedArray(data),w=targetW,h=targetH,sharpFactor=Math.max(0,Math.min(1,sharpness));for(let y=1;y<h-1;y++){const row=y*w;for(let x=1;x<w-1;x++){const idx=(row+x)*4,a=copy[idx+3];if(denoiseAlpha&&a<14){data[idx]=0;data[idx+1]=0;data[idx+2]=0;data[idx+3]=0;continue}if(a<25)continue;const up=((y-1)*w+x)*4,down=((y+1)*w+x)*4,left=(row+x-1)*4,right=(row+x+1)*4;for(let c=0;c<3;c++){const cVal=copy[idx+c],uVal=copy[up+c],dVal=copy[down+c],lVal=copy[left+c],rVal=copy[right+c],minVal=Math.min(cVal,uVal,dVal,lVal,rVal),maxVal=Math.max(cVal,uVal,dVal,lVal,rVal),contrast=Math.min(minVal,255-maxVal),amp=contrast/Math.max(maxVal,1),weight=-Math.sqrt(amp)*sharpFactor*0.22,sumNeighbors=uVal+dVal+lVal+rVal,sharpened=(cVal+weight*sumNeighbors)/(1+4*weight);data[idx+c]=Math.max(0,Math.min(255,Math.round(sharpened)))}}}ctx.putImageData(imgData,0,0);resolve(canvas.toDataURL('image/webp',.96))}catch(err){reject(err)}};img.onerror=reject;img.src=source})}
