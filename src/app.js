...
const PROFILE_ABILITIES={
  ...
  Vigilante:{id:'prontidao',profileAbility:'Vigilante',themeLinked:true,schema:'abilities',title:'PRONTIDÃO',type:'Habilidade de Perfil',content:'Você possui uma [color=#63bd42]**barra de prontidão**[/color] com 3 espaços. No início de cada cena, role 3d6 e anote cada valor nos espaços da barra (qualquer espaço já preenchido é substituído). Você também pode gastar [color=#63bd42]**2 PD**[/color] para preencher um espaço da barra.\n\nQuando você ou um aliado próximo faz um teste, você pode gastar um valor da barra para substituir um dado rolado pelo valor gasto.\nAlternativamente, você também pode gastar dois espaços da barra para, em qualquer ação ou conflito, [color=#63bd42]**agir antes de todos**[/color].',accent:'#63bd42',icon:'◆',markers:[{id:'prontidao-m',type:'slots',displayStyle:'rectangles',label:'Prontidão',current:0,maxFormula:'3',slots:[null,null,null],fillCostResource:'pd',fillCostAmount:2}],actions:[{id:'prontidao-a',name:'Rolar Prontidão',expression:'3d6',targetMarker:'prontidao-m'}]},
...
function getCriticalState(){
 const resource=(current,maximum)=>{const max=Math.max(0,Number(maximum)||0),value=Math.max(0,Number(current)||0),ratio=max>0?Math.min(1,value/max):1,critical=max>0&&ratio<=.25;return{value,max,ratio,critical,intensity:critical?Math.min(1,.3+(.25-ratio)/.25*.7):0}};
 const pv=resource(state.pv,state.maxPv),pd=resource(state.pd,state.maxPd);
 return{pv,pd,active:pv.critical||pd.critical};
}
function renderCriticalAtmosphere(){return `<div class="critical-atmosphere" data-critical-atmosphere aria-hidden="true"><div class="critical-pv-vignette"></div><div class="critical-pd-vignette"></div><canvas class="critical-tendrils" data-critical-tendrils></canvas><div class="critical-distortion"></div></div>`}
...
function playGlitchSound(){const ctx=getUiAudioContext();if(!ctx)return;const now=ctx.currentTime,buf=ctx.createBuffer(1,ctx.sampleRate*.42,ctx.sampleRate),data=buf.getChannelData(0);for(let i=0;i<data.length;i++){const env=i<2000?i/2000:(i>data.length-3000?(data.length-i)/3000:1);data[i]=(Math.random()*2-1)*env*(i%1700<60?2.5:1)}const src=ctx.createBufferSource(),bpf=ctx.createBiquadFilter(),dist=ctx.createWaveShaper(),gain=ctx.createGain();bpf.type='bandpass';bpf.frequency.setValueAtTime(1800,now);bpf.frequency.exponentialRampToValueAtTime(600,now+.12);bpf.Q.setValueAtTime(0.6,now);const curve=new Float32Array(256);for(let i=0;i<256;i++){const x=i/128-1;curve[i]=x*(1.2+Math.abs(x)*3.5)}dist.curve=curve;gain.gain.setValueAtTime(0.0001,now);gain.gain.linearRampToValueAtTime(0.38,now+.008);gain.gain.exponentialRampToValueAtTime(0.22,now+.05);gain.gain.exponentialRampToValueAtTime(0.001,now+.42);src.buffer=buf;src.connect(bpf).connect(dist).connect(gain).connect(ctx.destination);src.start(now)}

let criticalHeartbeatTimer=0,criticalHeartbeatActive=false,criticalHeartbeatIntensity=0,criticalPdAudio=null;
let criticalTendrilRaf=0,criticalTendrilCanvas=null,criticalTendrilIntensity=0,criticalTendrils=[];
const reducedMotionQuery=window.matchMedia?.('(prefers-reduced-motion: reduce)');

function emitCriticalHeartbeat(){
 const context=uiAudioContext;
 if(!context||context.state!=='running'||!criticalHeartbeatActive)return;
...
function setCriticalHeartbeat(active,intensity){
 ...
 const canPlay=active&&uiAudioContext?.state==='running';
 if(!canPlay){criticalHeartbeatActive=false;clearTimeout(criticalHeartbeatTimer);return}
...
function stopCriticalTendrils(){
 ...
 criticalTendrilCanvas=null;criticalTendrils=[];
}
function makeCriticalTendrils(width,height,intensity){
 const count=Math.round(7+intensity*12),short=Math.min(width,height);
 return Array.from({length:count},(_,index)=>{const side=index%4,r1=Math.random(),r2=Math.random(),r3=Math.random();let x,y,dx,dy;if(side===0){x=-4;y=r1*height;dx=1;dy=0}else if(side===1){x=width+4;y=r1*height;dx=-1;dy=0}else if(side===2){x=r1*width;y=-4;dx=0;dy=1}else{x=r1*width;y=height+4;dx=0;dy=-1}return{x,y,dx,dy,px:-dy,py:dx,length:short*(.16+intensity*.25)*(.68+r2*.64),amplitude:13+short*(.012+.023*intensity)*r3,frequency:2.2+r2*2.8,phase:r1*Math.PI*2,speed:.00045+r3*.00055,width:1.1+r2*2.3+intensity*1.4,branches:index%2===0?2:1,hue:index%3}})
}
function drawCriticalTendrils(canvas,time=0){
 ...
 if(canvas.width!==Math.round(width*dpr)||canvas.height!==Math.round(height*dpr)){canvas.width=Math.round(width*dpr);canvas.height=Math.round(height*dpr);context.setTransform(dpr,0,0,dpr,0,0);criticalTendrils=makeCriticalTendrils(width,height,criticalTendrilIntensity)}else context.setTransform(dpr,0,0,dpr,0,0);
 context.clearRect(0,0,width,height);context.globalCompositeOperation='screen';
 criticalTendrils.forEach(tendril=>{
  const points=[];
  for(let step=0;step<=24;step++){const progress=step/24,taper=Math.sin(progress*Math.PI),wave=Math.sin(progress*tendril.frequency*Math.PI+tendril.phase+time*tendril.speed)*tendril.amplitude*taper,hook=Math.sin(progress*Math.PI*.72+tendril.phase*.35)*tendril.amplitude*.38*progress;points.push({x:tendril.x+tendril.dx*tendril.length*progress+tendril.px*(wave+hook),y:tendril.y+tendril.dy*tendril.length*progress+tendril.py*(wave+hook)})}
  const tip=points.at(-1),gradient=context.createLinearGradient(tendril.x,tendril.y,tip.x,tip.y),colors=tendril.hue===0?['rgba(139,45,255,.88)','rgba(215,57,255,.6)']:tendril.hue===1?['rgba(86,47,255,.82)','rgba(78,190,255,.55)']:['rgba(201,37,234,.82)','rgba(105,61,255,.54)'];gradient.addColorStop(0,colors[0]);gradient.addColorStop(.58,colors[1]);gradient.addColorStop(1,'rgba(73,37,173,0)');
  const trace=()=>{context.beginPath();context.moveTo(points[0].x,points[0].y);for(let i=1;i<points.length;i++){const previous=points[i-1],point=points[i],midX=(previous.x+point.x)/2,midY=(previous.y+point.y)/2;context.quadraticCurveTo(previous.x,previous.y,midX,midY)}context.lineTo(tip.x,tip.y)};
  context.strokeStyle=gradient;context.lineCap='round';context.shadowColor=colors[0];context.shadowBlur=10+criticalTendrilIntensity*15;context.globalAlpha=.18+criticalTendrilIntensity*.25;context.lineWidth=tendril.width*3.4;trace();context.stroke();context.globalAlpha=.58+criticalTendrilIntensity*.3;context.lineWidth=tendril.width;trace();context.stroke();
  for(let branch=0;branch<tendril.branches;branch++){const at=Math.round(8+branch*6+tendril.hue),origin=points[Math.min(at,points.length-2)],direction=branch%2?1:-1,length=tendril.length*(.08+branch*.025),angle=time*tendril.speed*1.3+tendril.phase+branch;context.globalAlpha=.36+criticalTendrilIntensity*.22;context.lineWidth=Math.max(.7,tendril.width*.56);context.beginPath();context.moveTo(origin.x,origin.y);context.quadraticCurveTo(origin.x+tendril.dx*length*.35+tendril.px*direction*length*.35,origin.y+tendril.dy*length*.35+tendril.py*direction*length*.35,origin.x+tendril.dx*length*.62+tendril.px*direction*length*(.55+Math.sin(angle)*.12),origin.y+tendril.dy*length*.62+tendril.py*direction*length*(.55+Math.sin(angle)*.12));context.stroke()}
 });
...
function startCriticalTendrils(canvas,intensity){
 ...
 if(criticalTendrilCanvas===canvas&&sameSize&&Math.abs(criticalTendrilIntensity-nextIntensity)<.025)return;
 stopCriticalTendrils();criticalTendrilCanvas=canvas;criticalTendrilIntensity=nextIntensity;
 let lastFrame=0;const animate=time=>{if(criticalTendrilCanvas!==canvas)return;if(time-lastFrame>24){drawCriticalTendrils(canvas,time);lastFrame=time}criticalTendrilRaf=requestAnimationFrame(animate)};
 if(reducedMotionQuery?.matches)drawCriticalTendrils(canvas,0);else criticalTendrilRaf=requestAnimationFrame(animate);
}
function syncCriticalAtmosphere(){
 ...
 document.body.classList.toggle('critical-pv-state',critical.pv.critical);document.body.classList.toggle('critical-pd-state',critical.pd.critical);document.body.classList.toggle('critical-pv-zero',critical.pv.critical&&critical.pv.value===0);document.body.classList.toggle('critical-pd-zero',critical.pd.critical&&critical.pd.value===0);
 const atmosphere=document.querySelector('[data-critical-atmosphere]');
 if(atmosphere){atmosphere.classList.toggle('critical-pv',critical.pv.critical);atmosphere.classList.toggle('critical-pd',critical.pd.critical);atmosphere.classList.toggle('critical-combined',critical.pv.critical&&critical.pd.critical);atmosphere.style.setProperty('--pv-vignette-rest',(.13+critical.pv.intensity*.4).toFixed(3));atmosphere.style.setProperty('--pv-vignette-peak',(.25+critical.pv.intensity*.62).toFixed(3));atmosphere.style.setProperty('--pv-vignette-echo',(.2+critical.pv.intensity*.52).toFixed(3));atmosphere.style.setProperty('--pd-vignette-opacity',(.13+critical.pd.intensity*.42).toFixed(3));atmosphere.style.setProperty('--pd-tendril-opacity',(.28+critical.pd.intensity*.62).toFixed(3));atmosphere.style.setProperty('--pd-distortion-opacity',(.09+critical.pd.intensity*.3).toFixed(3));const canvas=atmosphere.querySelector('[data-critical-tendrils]');if(critical.pd.critical&&canvas)startCriticalTendrils(canvas,critical.pd.intensity);else stopCriticalTendrils()}else stopCriticalTendrils();
 const audioActive=cinematic&&visible;setCriticalHeartbeat(audioActive&&critical.pv.critical,critical.pv.intensity);setCriticalPdAudio(audioActive&&critical.pd.critical,critical.pd.intensity);
}
...
function bind(){
 document.querySelectorAll('[data-field]').forEach(el=>el.onchange=e=>{const key=e.target.dataset.field;state[key]=key==='level'?Math.max(0,Number(e.target.value)||0):e.target.value;save(false);render()});
 ...
 document.querySelectorAll('[data-attribute-step]').forEach(el=>el.onclick=()=>{const[name,delta]=el.dataset.attributeStep.split(':'),at=DICE_STEPS.indexOf(state.attributes[name]);state.attributes[name]=DICE_STEPS[Math.max(0,Math.min(DICE_STEPS.length-1,at+Number(delta)))];save(false);render()});
 ...
 document.querySelectorAll('[data-roll]').forEach(el=>el.onclick=e=>{if(e.target.closest('.custom-select,.extra input,[data-skill-extra],.card-actions,.skill-info-button,[data-remove-aptidao],[data-add-aptidao],.btn-remove-aptidao'))return;const[name,sides,bonus]=el.dataset.roll.split('|');roll(name,sides,bonus)});
  ...
  document.querySelectorAll('[data-use-slot]').forEach(el=>{el.onclick=()=>{const[cardId,markerId,slotIdxStr]=el.dataset.useSlot.split(':'),slotIndex=Number(slotIdxStr),card=state.cards.find(c=>c.id===cardId),marker=card?.markers.find(m=>m.id===markerId);if(!card||!marker)return;if(!Array.isArray(marker.slots))marker.slots=[];const val=marker.slots[slotIndex];if(val!==null&&val!==undefined&&val!==''){marker.slots[slotIndex]=null;marker.current=marker.slots.filter(v=>v!==null&&v!==undefined&&v!=='').length;playResourceSound(false);save(false);render();const toast=document.querySelector('#save-toast');if(toast){toast.textContent=`VALOR [ ${val} ] UTILIZADO!`;toast.classList.add('show');setTimeout(()=>{toast.classList.remove('show');toast.textContent='SALVO'},1800)}}else{openSlotModal(card,marker,slotIndex)}};el.oncontextmenu=e=>{e.preventDefault();const[cardId,markerId,slotIdxStr]=el.dataset.useSlot.split(':'),slotIndex=Number(slotIdxStr),card=state.cards.find(c=>c.id===cardId),marker=card?.markers.find(m=>m.id===markerId);if(card&&marker)openSlotModal(card,marker,slotIndex)}});
  document.querySelectorAll('[data-fill-marker]').forEach(el=>el.onclick=()=>{const[cardId,markerId]=el.dataset.fillMarker.split(':'),card=state.cards.find(c=>c.id===cardId),marker=card?.markers.find(m=>m.id===markerId);if(!card||!marker)return;const max=markerMax(marker);if(!Array.isArray(marker.slots))marker.slots=[];let targetIdx=-1;for(let i=0;i<max;i++){if(marker.slots[i]===null||marker.slots[i]===undefined||marker.slots[i]===''){targetIdx=i;break}}if(targetIdx===-1)targetIdx=0;openSlotModal(card,marker,targetIdx)});
...
window.addEventListener('resize',syncCriticalAtmosphere);
